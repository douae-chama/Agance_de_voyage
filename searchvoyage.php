...
if (isset($_POST['search'])) {
    ...
    $stmt = $connexion->prepare("SELECT * FROM Voyages WHERE Titre LIKE :searchTerm OR Destination LIKE :searchTerm");
...
<body>
  ...
  <a href="mesreservation.php" title="Mes reservation">
    ...
    <?php if (isset($voyages) && count($voyages) > 0): ?>
        <div class="cards-container">
            <?php foreach ($voyages as $voyage): ?>
                <div class="cart">
                    <?php
                    $image_path = "photo et logo/" . htmlspecialchars($voyage['image']);
                    if (!file_exists($image_path)) {
                        $image_path = "photo/default.jpg";
                    }
                    ?>
                    <img src="<?= $image_path ?>" alt="<?= htmlspecialchars($voyage['Titre']) ?>">
                    ...
                    <p><strong>Départ :</strong> <?= htmlspecialchars($voyage['DateDepart']) ?></p>
                    <p><strong>Retour :</strong> <?= htmlspecialchars($voyage['DateRetour']) ?></p>
                    <p class="price"><?= htmlspecialchars($voyage['Prix']) ?> dh</p>
                    <a href="reservationvoyages.php?id_voyage=<?= $voyage['IdVoyage'] ?>"><button>Réserver</button></a>
                </div>
            <?php endforeach; ?>
        </div>
    <?php else: ?>
        <p class="no-results">Aucun voyage trouvé pour votre recherche.</p>
    <?php endif; ?>
